...
output: 5 # less than 5 kWh and the drone would crash before the finish
          # line. More than `5` kWh and it’d end 

*/

/* 
    - Essentially you only care about the drone going up and down. 
    - The z index is the only coordinate which indicates ascends and descends
    - You're trying to find from the starting point (10) the shortest trip to complete it's flight (15 - max = completion)
    - So take the starting, take the trip end and find the difference between the two
*/
function calcDroneMinEnergy(route) {
  ...
  // your code goes here
  let temp = [];
  route.forEach(r => temp.push(r[2]));

  // temp = [10,0,6,15,8]
  let start = route[0][2];
  let end = Math.max.apply(null, temp);
  console.log(temp, start, end);
