...
output: 1487800378 # since the increase in the number of people
                   # in the mall is the highest a
*/

function findBusiestPeriod(input) {
  // condense data to eliminate duplicate time stamps
  let enter = 1;
  let maxTime = null;
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    count = entry === enter ? (count += visitors) : (count -= visitors);

    if (i < input.length - 1 && input[i][0] === input[i + 1][0]) continue;
...
module.exports = findBusiestPeriod;
